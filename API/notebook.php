<?php
$connect = mysqli_connect('mysql', 'root', 'root', 'users');
$post  = $_POST;

if ($_SERVER['REQUEST_METHOD'] === 'GET' && !empty($ur[3])){
    $dateJSGET = array();
    $idGET = intval($ur[3]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($ur[3])){
    $idPOST = intval($ur[3]);
    $setClauses = array();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $email = $post['email'];
    mysqli_query($connect, "insert into `data`(`username`, `lastname`, `phoneNumber`, `email`)
values ('$username', '$lastname', '$phonenumber', '$email')");
    $ok = array('ok' => 1);
    echo json_encode($ok);
    die();
}
